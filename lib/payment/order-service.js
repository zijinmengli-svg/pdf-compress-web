...
function createOrderService({ repo, pool, identityHashSecret, now = () => new Date(), transaction = withTransaction }) {
  ...
  return {
    async registerCompressionSuccess(input) {
      ...
      return transaction(pool, async (tx) => {
        ...
        await appendLifecycleEvent(tx, { orderId, eventType: free ? "free_grant_consumed" : "payment_required", paymentStatus, fulfillmentStatus, currency: priceCurrency, occurredAt: timestamp });
