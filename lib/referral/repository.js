...
function createReferralRepository({ pool, now = () => new Date(), timezone = "Asia/Shanghai" }) {
  ...
  return {
    ...
    async listAdminEvents({ limit = 100, status = "" } = {}, db = pool) {
      ...
      const columns = "id, status, first_compression_job_id, first_download_token_id, first_download_at, blocked_reason, created_at, updated_at";
      if (status) return (await db.query(`SELECT ${columns} FROM referrals WHERE status = $1 ORDER BY updated_at DESC LIMIT $2`, [status, bounded])).rows;
      return (await db.query(`SELECT ${columns} FROM referrals ORDER BY updated_at DESC LIMIT $1`, [bounded])).rows;
    },
