...
function uploadKey(event) {
  ...
  const uploadAttemptId = String(data.uploadAttemptId || "").trim();
  const jobId = String(data.jobId || "").trim();
  const file = String(data.fileName || "").toLowerCase();
  return `${actor}|${uploadAttemptId || jobId || file}`;
}

function isCompressionTaskEvent(event) {
  const data = eventData(event);
  if (!data.fileName || !["compress_started", "compress_success", "compress_error", "download_clicked"].includes(event.event)) {
    return false;
  }
  if (event.event !== "compress_started") return true;
  return Boolean(String(data.uploadAttemptId || "").trim() || String(data.jobId || "").trim());
}
...
const PROCESSING_SLOW_MS = Math.max(
  60_000,
  Number(process.env.ANALYTICS_PROCESSING_SLOW_MS) || 10 * 60_000
);
const PROCESSING_STALE_MS = Math.max(
  35 * 60_000,
  Number(process.env.ANALYTICS_PROCESSING_STALE_MS) ||
    Number(process.env.ANALYTICS_PROCESSING_TIMEOUT_MS) ||
    35 * 60_000
);
...
function mergeRecentFileEvents(events, now = new Date()) {
  ...
  for (const event of events) {
    const data = eventData(event);
    if (!isCompressionTaskEvent(event)) continue;
    const key = uploadKey(event);
    ...
    if (event.event === "compress_error") {
      existing.status = data.code === "COMPRESSION_TIMEOUT" ? "timeout" : "error";
  ...
  for (const row of rows.values()) {
    ...
    if (row.status !== "processing" || !Number.isFinite(rowTime)) continue;
    if (rowTime <= staleAt) {
      row.status = "lost";
      row.reason = "任务失联：超过35分钟未收到完成事件";
    } else if (rowTime <= now.getTime() - PROCESSING_SLOW_MS) {
      row.status = "slow";
      row.reason = "仍在处理，大文件可能耗时较长";
    }
  }
  return [...rows.values()]
...
function summarizeAnalytics(events, now = new Date()) {
  ...
  const startedTaskCount = Math.max(
    dedupeCount(last30d, event => isCompressionTaskEvent(event)),
    completedTaskCount
  );
