    ...
    async registerCompressionSuccess(input) {
      ...
      return transaction(pool, async (tx) => {
        ...
        if (creditService && input.walletHash) {
          const credit = await creditService.consumeForCompression({ walletHash: input.walletHash, legacyIdentityHash: input.legacyIdentityHash || identityHash, jobId: input.jobId }, tx);
