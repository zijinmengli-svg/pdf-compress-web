...
function summarizeAnalytics(events, now = new Date()) {
  ...
  const fileEvents = last30d.filter(event => event.event === "file_selected");
  ...
  return {
    ...
    funnel: {
      ...
      file_selected: last30d.filter(event => event.event === "file_selected").length,
      compress_started: last30d.filter(event => event.event === "compress_started").length,
    ...
    compression: {
      uploadSizeBuckets: countBy(fileEvents, event => bucketMB(eventData(event).fileBytes), "bucket"),
      targetSizeBuckets: countBy(last30d.filter(event => event.event === "compress_started"), event => bucketMB(Number(eventData(event).targetMB || 0) * 1048576), "bucket"),
